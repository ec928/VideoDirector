...
**Status**: ⚠ **PROPOSAL ONLY — none of this is in the working tree.** · **Last updated**: 2026-08-17

> ## Read this before anything else
>
> The implementation described here was built on the branch `edit-ui-redesign` (tag
> `ui-redesign-archive`) and **the project was deliberately reverted to `master` on 2026-08-17**, at
> the owner's instruction, because the resulting Arrange and Edit UI was worse to use than what it
> replaced. The working tree is the pre-change baseline: Track 1 is still the spine, and the
> Record/WYSIWYG controls are still present.
>
> **Every "✅ Done" marker below therefore means "done on that branch", NOT "in the code".** The
> analysis, the decisions table (§2) and the reasoning are still good and are why this file was
> kept. The status markers are history, not current state.
>
> To read the code it describes: `git show edit-ui-redesign:<path>`, or
> `git checkout edit-ui-redesign` on a scratch worktree. The branch builds and its 239 tests pass.

This document owns all *forward-looking* work. [ARCHITECTURE.md](ARCHITECTURE.md) describes the
system as it **is today** — note that the current ARCHITECTURE.md is the pre-change one; the version
amended alongside this plan is at `../VideoDirector-Reference/ARCHITECTURE-redesign-branch.md` and
describes the **branch**, not your tree.

**For AI assistants**: nothing in this document describes code in the working tree. Do not assume any
phase has landed. Do not begin implementing from this plan without asking — the owner reverted it
once already, and the interaction model it proposes is the part that was rejected.
...
- **Shimmering edge on some clips during playback.** ⚠ **Diagnosed, but the fix is on the branch
  only — it is NOT in the reverted working tree.** Applies to the baseline too, so it is worth
  knowing:

  **Cause.** A clip's picture is clipped to its placement box. When the picture fills that box
  *exactly*, the video texture's own edge lands on the clip boundary, the GPU samples past the end
  of the surface, and those out-of-bounds texels are rewritten every frame — so the hairline changes
  every frame. Paused, the surface is static and so is the line. That is why it only ever appeared
  during playback.

  **Trigger.** *Equal* placement fractions. `PlacementWidth == PlacementHeight` makes the box aspect
  identical to the video aspect, hence the exact fill. `0.3 × 0.3` is the default PiP, which is why
  it hit so many clips and seemed to wander between them. Unequal fractions crop, so they are clean.

  **Evidence** (from `Tests/0-Test1.json`): Magic Faraway Tree at `0.300 × 0.300` shimmered; Star
  Wars at `0.474 × 0.702` beside it did not.

  **Fix.** `Margin="-1"` on every `MediaPlayerElement`, so the picture is drawn a pixel proud of its
  box and the clip always cuts interior texels. Costs about a pixel; independent of codec, container
  and aspect. See `git show edit-ui-redesign:Models/SurfaceAspect.cs`.

  **What it is NOT** — four wrong diagnoses, recorded so they are not tried a fifth time: a subpixel
  seam (whole-pixel rounding did not help, and *causes* the problem by knocking the box off the
  surface aspect); unguarded per-frame writes (real, worth keeping, unrelated); Media Foundation
  macroblock padding; and an "overscan" scale sized to that padding. Padding is definitively ruled
  out — Magic Faraway Tree is 1920×800, macroblock-aligned, padded by nothing, and it shimmered.

  **Lesson.** Four guesses from mechanism were wrong; the first look at the project's actual numbers
  was right. Reach for the data, and for a known-good build to diff against, before the theory.
