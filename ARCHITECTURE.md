...
* **Decoder Padding Cropped Off Clip Edges**: Media Foundation reports the DECODED frame size, padded out to macroblock multiples — a 1918x804 video comes back as 1920x816. Shaping a clip's box from that padded figure left a strip of codec padding along the edge, and padding pixels change frame to frame, which read as a shimmering border. The file's own `SourceAspect` now wins over the decoder's figure everywhere, and `OverscanFor` scales the picture by the mismatch so the padding falls outside the box. Exactly 1.0 — no change at all — when the decoder padded nothing, which is why most videos never showed it.
