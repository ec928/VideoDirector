...
* **A Video Box Takes The Decoder's Shape (the shimmering edge)**: `MediaPlayerElement` scales its picture from the DECODED frame, which Media Foundation pads out to macroblock multiples — a 1918x804 file arrives as 1920x816. Shaping a clip's box from the file's real aspect instead makes the two disagree, so `UniformToFill` overflows and the grid's `RectangleGeometry` clip has to cut a live GPU swapchain; the compositor re-resolves that boundary against a surface being rewritten at frame rate, and the edge crawls. A box shaped from the decoded aspect overflows by exactly zero, so there is nothing to clip. It costs a couple of padding pixels along one edge, which is what the app showed for its whole working life. Stills keep the file's real aspect — a bitmap has no surface to crawl. Rule in `Models/SurfaceAspect.cs`. **Two earlier "fixes" for this were compensating for a mismatch that should not have existed and are gone**: an `OverscanFor` scale, and `Math.Round` on box width/height (which snapped the box off the surface aspect by up to a pixel, recreating the bug in miniature). Note the diagnostic history — three plausible mechanisms were wrong; the answer came from diffing a known-good build.
* **Per-Frame Write Guards Extended To The Whole Render Path**: `SetOverlayRender` wrote four properties per track per frame — still visibility, frame visibility, video visibility and grid opacity — unguarded, which reintroduced the flickering hairline down the edge of a PiP during playback. All of them, plus the constant-valued half of the crossfade and track 0's player opacity, now go through delta-guarded setters. See invariant §5.6.
...
6. **GPU Surface Preservation**: Never directly manipulate live `MediaPlayerElement` XAML properties per-frame in ways that corrupt DirectX swapchains; use dedicated transforms and delta-checked placement boxes. Corollary (§7A): never reshape a live video surface — Arrange shows a still proxy instead.
   **Every write in the render path must be delta-guarded**, not only the box and the transform. Assigning a XAML property that has not changed still dirties the DirectComposition visual tree, and at 60fps on the grid wrapping a live video surface that shows as a flickering hairline down the edge of a PiP. Use `SetVisibility` / `SetOpacity` / `SetImageSource` in `VideoPlaybackEngine` rather than assigning directly.
