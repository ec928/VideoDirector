...
* **Output Follows The Canvas.** The render was pinned at 1920×1080 while a project canvas can be 4K, 2.39:1, 9:16 or anything typed, so a vertical project was squeezed into landscape by a renderer told the wrong shape. The profile now carries the real frame size, and overlay positions resolve into it. Verified end to end by linking the SHIPPING exporter and model into a headless harness and rendering a real two-track project (trimmed clips, a fade, a PiP at 30%% size and half opacity): Success at 1080×1920, 2560×1072 and an odd 1001×667 correctly rounded to 1000×668 for H.264. Sizes read back out of the files’ `tkhd`, and a frame pulled from each and looked at — the PiP lands within a pixel or two of its computed box. Neither the encoder honouring a frame size nor a composite being in the right place is something a compiler can check. The only export surface not exercised this way is the click handler and the file picker.
