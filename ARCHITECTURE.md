...
* **Output Follows The Canvas.** The render was pinned at 1920×1080 while a project canvas can be 4K, 2.39:1, 9:16 or anything typed, so a vertical project was squeezed into landscape by a renderer told the wrong shape. The profile now carries the real frame size, and overlay positions resolve into it. Verified by parsing `tkhd` out of the rendered files: 1080×1920, 2560×1072 and 3840×2160 all come back correct — the encoder honouring the size is not something a compiler can check.
